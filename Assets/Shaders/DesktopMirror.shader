	...
	Properties{
		...
		SubShader{
			...
			void vert(inout appdata_full v, out Input o) {
				...
				o.desktopUV1.x = (v.color.x * 255) / (_XGridSize - 1);
				o.desktopUV1.y = (v.color.y * 255) / (_YGridSize - 1);
				/*o.desktopUV2.x = (v.color.x * 255) / _XGridSize;
				o.desktopUV2.y = (v.color.y * 255) / _YGridSize;
				*/
			...
			void surf(Input IN, inout SurfaceOutput o) {
				o.Alpha = 1;
				fixed4 c = tex2D(_MainTex, IN.desktopUV1) * _Color;
				fixed4 c2 = tex2D(_MainTex2, IN.desktopUV1) * _Color;
				if (IN.desktopUV1.x < 1)
				{
					o.Albedo = c.rgb;
				}
				else if (IN.desktopUV1.x >= 1 && IN.desktopUV1.x <=	2) {
					o.Albedo = c2.rgb;
				}
				else {
					o.Albedo = (0, 0, 0);
