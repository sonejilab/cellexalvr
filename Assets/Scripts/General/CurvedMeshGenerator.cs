...
public class CurvedMeshGenerator : MonoBehaviour
{
    ...
    public void GenerateNodes(float r = 1.0f, float curvature = 0.3f)
    {
        if (r == 0) r = 1;
    ...
    {
        double yAngle = 2 * Math.PI;
        for (int y = 0; y < ySize; y++)
        {
            float yPos = (float)Math.PI * y / (float)(ySize); //(float) (Math.Cos(yAngle)) + 1;
            float zPos = (float)(Math.Sin(yAngle)) * curvatureY;// + (float)(Math.Cos(;
            Vector3 p2 = new Vector3(0, yPos, zPos);
            for (int x = 0; x < xSize; x++)
            {
                float xPos = p2.x + (float)(radius * Math.Cos((x + xSize / 2) * -2 * Math.PI * curvatureX / xSize));
                yPos = p2.y;
                zPos = (float)(radius * Math.Sin((x + xSize / 2) * -2 * Math.PI * curvatureX / xSize)); // * curvatureX);
    ...
    private void GenerateSphericalNodes()
    {
