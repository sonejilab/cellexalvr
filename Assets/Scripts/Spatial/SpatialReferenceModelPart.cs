...
namespace Spatial
{
    public class SpatialReferenceModelPart : MonoBehaviour
    {
        ...
        private List<Mesh> allParts = new List<Mesh>();
        ...
        public void SplitMesh()
        {
            ...
            List<Mesh> parts = CellexalVR.MeshUtils.MeshSplitter.SplitMesh(meshFilter.mesh);
        ...
        public IEnumerator Spread()
        {
            if (id != 997)
            {
                ...
                Vector3[] dirs = new Vector3[mcs.Length];
                Vector3[] start = new Vector3[mcs.Length];
                Vector3[] end = new Vector3[mcs.Length];
                for (int i = 0; i < mcs.Length; i++)
                {
                    start[i] = new Vector3(mcs[i].transform.position.x, mcs[i].transform.position.y, mcs[i].transform.position.z);
                    dirs[i] = (mcs[i].bounds.center - center).normalized;
                    if (spread)
                    {
                ...
                while (t < animationTime)
                {
                    ...
                    for (int i = 0; i < mcs.Length; i++)
                    {
                        mcs[i].transform.position = Vector3.Lerp(start[i], end[i], progress);
