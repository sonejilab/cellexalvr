...
namespace CurvedVRKeyboard
{
    ...
    public class KeyboardCreatorEditor : Editor
    {
        ...
        private static GUIContent KEY_TEXT_MATERIAL_CONTENT = new GUIContent("Key text material", "Material used with text on keys");
        private static GUIContent KEY_FONT_CONTENT = new GUIContent("Key font", "Font used with text on keys");
        ...
        private void Awake()
        {
            ...
            {        // Always when not playing or (playing and keyboard is not static)
        ...
        public override void OnInspectorGUI()
        {
            ...
            {        // (Playing and was static at start) or always when not playing
                DrawPrimary();
                DrawMaterials();
                DrawSpace();
                GUI.enabled = true;
        ...
        private void DrawMaterials()
        {
            ...
            keyboardCreator.keyTextMaterial = EditorGUILayout.ObjectField(KEY_TEXT_MATERIAL_CONTENT, keyboardCreator.keyTextMaterial, typeof(Material), true) as Material;
            keyboardCreator.keyFont = EditorGUILayout.ObjectField(KEY_FONT_CONTENT, keyboardCreator.keyFont, typeof(Font), true) as Font;
        }
        ...
        private void CameraFinder()
        {
            ...
            {
                if (GUILayout.Button(FIND_SOURCE_BUTTON))
                {        //if seach camera button press
                    SearchForCamera();
                }
                if (noCameraFound)
                {         // After button press there is no camera
        ...
        private void HandleScaleChange()
        {
            ...
            if (keyboardCreator.transform.localScale.x != keyboardScale.x)
            {         // X scale changed
                neededXScale = keyboardCreator.transform.localScale.x;
            }
            else if (keyboardCreator.transform.localScale.z != keyboardScale.z)
            {         // Z scale changed
