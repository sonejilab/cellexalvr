...
namespace CurvedVRKeyboard
{
    ...
    public class KeyboardCreator : KeyboardComponent
    {
        ...
        public void CheckErrors()
        {
            ...
            if (keys.Length != KEY_NUMBER)
            {        //is there correct number of keys
                errorReporter.SetMessage("Cannot procced. Number of keys is incorrect. Revert your changes to prefab. Number of keys: " + keys.Length, ErrorReporter.Status.Error);
                return;
            }

            if (space == null)
            {         // project improted over older package crashes without this
                space = keys[spaceKeyNumber];
            }
            if (space.GetMeshName().Equals(MESH_NAME_SEARCHED))
            {        //are keys positioned corectly
            ...
            {        //is playing and was static when play mode started
